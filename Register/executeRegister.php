...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validation
    if (empty($cpf_no) || empty($name) || empty($email) || empty($department) || empty($password)) {
    ...
    if (empty($errors)) {
        try {
            ...
            $sql = "INSERT INTO user (cpf_no, name, email, department, hashed_password, designation, phone_number) 
                    VALUES (?, ?, ?, ?, ?, ?, ?)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$cpf_no, $name, $email, $department, $hashed_password, $designation, $phone_number]);
